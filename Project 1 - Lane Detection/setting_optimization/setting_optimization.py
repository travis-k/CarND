...
def draw_lines(img, lines, y_max, x_max, color=[255, 0, 0], thickness=2):
    ...
    if lines is not None: # Simple check to see if there are indeed lines
        ...
        # The lengths of all the lines are found as well
        ...
        left_side = (slopes > 0)
        # If a point from the "left lane" is on the right half of the image, we will ignore it as it is probably incorrect or noise
        offside = np.logical_or(lines[:,0,0] < x_max/2, lines[:,0,2] < x_max/2) 
        left_side[offside] = False
        
        # We do the opposite of the above procedure to find the right lane lines
        right_side = (slopes < 0)
        offside = np.logical_or(lines[:,0,0] > x_max/2, lines[:,0,2] > x_max/2)
        right_side[offside] = False
        
        # These provide us with all the lines on the left, and the right, separately
        left_lines = lines[left_side,:,:]
        right_lines = lines[right_side,:,:]
        
        # Now we draw the lines on, first left then right in two separate loops
...
def round_up_to_odd(f):
    ...
    # This function simply rounds up to the next odd integer, and is used for the kernel for Gaussian blur
    f = int(np.ceil(f))
...
def process_image(img, kernel_size, low_threshold, high_threshold, rho, theta, threshold, min_line_len, max_line_gap):
    ...
    # We return the Hough Line image, because this is what we compare against (red on black)
    return line_image
    
def objective_function(design_vars):
    # This function takes in the above design variables and will return an image with a black background and red ([255 0 0]) lines. This will be compared to the desired image, created manually, and the end result of the optimization will be settings which hopefully provide the most similar image to the desired result.
    
    # The input is a vector of values, so here we assign them to the appropriate variable
    ...
    # Getting the input locations for the test image, and the desired hough line image which I created manually
    strImageIn = ["test_images/" + x for x in os.listdir("test_images/")]
    strImageDesired = ["desired_images/" + x for x in os.listdir("test_images/")]
    
    likeness = []
    
    # Running through all 6 test images and comparing them
    for i in range (0,5):
        ...
        # The lower the "likeness" value, the better the images match
        likeness.append(mse(marked_lane, img_desired))

    # Python doesn't have multi-objective optimization readily available??? So I'm averaging the results from the 6 images. This is not so desireable
...
# # These are the bounds of the design variables
...
# Visualizing the results and creating images
